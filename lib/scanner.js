...
export function scan(scanPaths, options) {
  ...
  const classifyFile = (full, ctxKind) => {
    ...
    if (HTML_EXT.has(ext)) {
      ...
      const rec = parseHtml(full, c, { hasManifest });
      // Files in the documents folder (e.g. from Promote) are always standalone & editable.
      if (opts.documentsDir && full.startsWith(expandHome(opts.documentsDir))) {
        rec.subtype = 'document';
        rec.readOnly = false;
        rec.classification = { class: 'document', confidence: 1, reasons: ['in your documents folder — standalone & editable'] };
      }
      scanned++; add(rec, full); return;
    }
